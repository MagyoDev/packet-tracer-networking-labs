...
Neste laboratório, pratiquei a configuração inicial de switches Cisco e entendi como preparar um switch para administração básica.

Os principais aprendizados foram:

- Diferença entre `running-config` e `startup-config`
- Uso de SVI para gerenciamento
- Configuração de senhas de acesso
- Importância do comando `login`
- Uso de descrições em interfaces
- Verificação de conectividade com `ping`
- Análise da tabela MAC
- Uso de comandos `show` para troubleshooting
